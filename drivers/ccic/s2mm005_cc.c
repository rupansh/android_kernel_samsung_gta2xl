...
#if (defined(CONFIG_SEC_GTA2XLLTE_PROJECT) || defined(CONFIG_SEC_GTA2XLWIFI_PROJECT)) && !defined(CONFIG_MACH_GTA2XLLTE_USA_VZW)
			// This W/A codes for Tab A2 XL with MPA2+Charger connection.
			if( usbpd_data->func_state & (0x1 << 25) && usbpd_data->func_state & (0x1 << 26)
		...
		case State_PE_SRC_Disabled:
			...
			if (usbpd_data->is_host == HOST_OFF) {
				...
				if (!is_blocked(o_notify, NOTIFY_BLOCK_TYPE_HOST)) {
					vbus_turn_on_ctrl(1);
#if (defined(CONFIG_SEC_GTA2XLLTE_PROJECT) || defined(CONFIG_SEC_GTA2XLWIFI_PROJECT)) && !defined(CONFIG_MACH_GTA2XLLTE_USA_VZW)
					if(usbpd_data->pd_state == State_PE_SRC_Send_Capabilities) {
						s2mm005_set_vbus_status(STATUS_VBUS_HIGH, State_PE_SRC_Send_Capabilities);
					}
#endif 
				}
