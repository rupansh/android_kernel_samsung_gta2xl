...
def rem_qcom(args):
    split(args)
    print(os.listdir("dtb"))
    files = list()
    for dtb in os.listdir("dtb"):
        if not (("XL" in dtb) or ("00_kernel" in dtb)):
            os.remove("dtb/"+dtb)
            print("removed ", "dtb/"+dtb)
    for infile in os.listdir("dtb"):
        print(infile)
        files.append("dtb/"+infile)

    os.system("cat "+" ".join(files)+" > "+args.filename)

    shutil.rmtree("dtb")

    print(args.filename, "ready!")
