...
tfa9896_tfa_dsp_write_tables(tfa98xx_handle_t dev_idx, int sample_rate)
{
	...
	/* Not used for max1! */
//	sample_rate = sample_rate; (retards?)
...
static enum tfa98xx_error
tfa9896_faim_protect(tfa98xx_handle_t handle, int status)
{
	enum tfa98xx_error ret = TFA98XX_ERROR_OK;

	if ((handles_local[handle].rev  == 0x2b96)
		|| (handles_local[handle].rev == 0x3b96)) {
		ret = tfa_set_bf_volatile(handle,
			TFA9896_BF_OPENMTP, (uint16_t)status);
	}
...
static enum tfa98xx_error tfa9894_specific(tfa98xx_handle_t handle)
{
	enum tfa98xx_error error = TFA98XX_ERROR_OK;
	unsigned short value, xor;
	...
	/* The optimal settings */
	if (handles_local[handle].rev == 0x0a94) {
		/* V36 */
		/* ----- generated code start ----- */
		reg_write(handle, 0x00, 0xa245); /* POR=0x8245 */
		reg_write(handle, 0x02, 0x51e8); /* POR=0x55c8 */
		reg_write(handle, 0x52, 0xbe17); /* POR=0xb617 */
		reg_write(handle, 0x57, 0x0344); /* POR=0x0366 */
		reg_write(handle, 0x61, 0x0033); /* POR=0x0073 */
		reg_write(handle, 0x71, 0x00cf); /* POR=0x018d */
		reg_write(handle, 0x72, 0x34a9); /* POR=0x44e8 */
		reg_write(handle, 0x73, 0x3808); /* POR=0x3806 */
		reg_write(handle, 0x76, 0x0067); /* POR=0x0065 */
		reg_write(handle, 0x80, 0x0000); /* POR=0x0003 */
		reg_write(handle, 0x81, 0x5715); /* POR=0x561a */
		reg_write(handle, 0x82, 0x0104); /* POR=0x0044 */
		/* ----- generated code end   ----- */
	} else if (handles_local[handle].rev == 0x1a94) {
		/* V7 */
		/* ----- generated code start ----- */
		reg_write(handle, 0x00, 0xa245); /* POR=0x8245 */
		reg_write(handle, 0x02, 0x5288); /* POR=0x55c8 */
		reg_write(handle, 0x52, 0xbe17); /* POR=0xb617 */
		reg_write(handle, 0x56, 0x05c3); /* POR=0x07c3 */
		reg_write(handle, 0x57, 0x0344); /* POR=0x0366 */
		reg_write(handle, 0x61, 0x0032); /* POR=0x0073 */
		reg_write(handle, 0x71, 0x00cf); /* POR=0x018d */
		reg_write(handle, 0x72, 0x34a9); /* POR=0x44e8 */
		reg_write(handle, 0x73, 0x3808); /* POR=0x3806 */
		reg_write(handle, 0x76, 0x0067); /* POR=0x0065 */
...
static enum tfa98xx_error
tfa9894_set_mute(tfa98xx_handle_t handle, int mute)
{
	tfa_set_bf(handle, TFA9894_BF_CFSM, (uint16_t)mute);

	return TFA98XX_ERROR_OK;
}

static enum tfa98xx_error
tfa9894_dsp_system_stable(tfa98xx_handle_t handle, int *ready)
{
	enum tfa98xx_error error = TFA98XX_ERROR_OK;

	/* check CLKS: ready if set */
	*ready = tfa_get_bf(handle, TFA9894_BF_CLKS) == 1;

	return error;
}

static enum tfa98xx_error
tfa9894_faim_protect(tfa98xx_handle_t handle, int status)
{
	enum tfa98xx_error ret = TFA98XX_ERROR_OK;

	/* 0b = FAIM protection enabled, 1b = FAIM protection disabled */
	ret = tfa_set_bf_volatile(handle,
		TFA9894_BF_OPENMTP, (uint16_t)(status));
 ...
 */
static enum tfa98xx_error
tfa9894_set_osc_powerdown(tfa98xx_handle_t handle, int state)
{
	enum tfa98xx_error ret = TFA98XX_ERROR_OK;

	if (state == 1 || state == 0)
		ret = tfa_set_bf(handle, TFA9894_BF_MANAOOSC, (uint16_t)state);
	else
...
void tfa9894_ops(struct tfa_device_ops *ops)
{
	...
	ops->tfa_dsp_system_stable = tfa9894_dsp_system_stable;
